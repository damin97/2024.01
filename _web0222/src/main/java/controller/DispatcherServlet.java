...
//			ArrayList<Board> list = dao.selectList(); // selectList() 호출
			//포워딩 작업
//			request.setAttribute("list", list); //ArrayList를 넣을 수 있다
			...
			Board board = dao.selectOne(num, true); //값을 가져오는 것
			...
			if(member != null) {
				...
				session.setAttribute("member", member); // 세션에 값 저장
		    ...
		    request.setAttribute("board", board); //포워딩하는 것: 뷰에 값을 주기 위해 하는 작업
		    request.setAttribute("action", action);//포워딩 작업
